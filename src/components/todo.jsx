import { handleDelete, handleToggle} from '../handler'

export default ({ state, set }, [todo]) => {
  const { todos } = state
  ...
  return (
    <li className="todo">
      ...
      <div className="actions">
        <button
          className={`btn ${todo.done ? 'gray' : 'green'}`}
          onClick={() => handleToggle(set, todos, todo.id)}
          >
          {todo.done ? (
        ...
        <button
          className="btn red"
          onClick={() => handleDelete(set, todos, todo.id)}
          >
